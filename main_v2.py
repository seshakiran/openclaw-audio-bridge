"""
Voice Assistant v2 - Using MLX TTS and VAD-based STT.
Features:
- MLX-based TTS (ElevenLabs quality, local)
- VAD-based recording (only captures speech, reduces echo)
...
# Use the new modules
from stt_vad_module import STTVADHandler
from tts_mlx_module import (
    speak, speak_sync, stop_speech, is_speaking, 
    get_recent_texts, clear_recent_texts, wait_for_speech,
    preload_model
)
from brain_module_v2 import brain
...
def set_mode(mode):
    ...
    # Notify STT handler about output state
    if stt_handler:
        stt_handler.set_output_active(mode in ['speaking', 'processing'])
...
def mark_speech_ended():
    ...
    if stt_handler:
        stt_handler.set_output_active(False)
...
def is_wake_word(text):
    lower = text.lower()
    wake_words = ["marvin", "marlin", "martin", "marving", "marvel", "marin", "marben", "marv"]
...
async def main():
    ...
    print("=" * 60)
    print("🚀 Voice Assistant v2 - MLX TTS + VAD STT")
    print("=" * 60)
    ...
    # Initialize VAD-based STT
    print("Loading STT model...")
    stt_handler = STTVADHandler(model_name="base", device_index=config.MICROPHONE_INDEX)
